...
export function KitchenProductDetailPage({ product }: Props) {
  const { locale, t } = useLocale()
  const copy = React.useMemo(
    () => getKitchenProductCopy(product, locale),
    [product, locale],
  )
  ...
  const supportBullets = React.useMemo(
    () =>
      [
        t('kitchen.detailSupportLi1'),
        t('kitchen.detailSupportLi2'),
        t('kitchen.detailSupportLi3'),
      ] as const,
    [t],
  )
  ...
  return (
    <>
            ...
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Button variant="premium" size="lg" asChild>
                  <Link href={routes.contact}>{t('kitchen.detailCtaSpecs')}</Link>
                </Button>
                <Button variant="outline" size="lg" asChild>
                  <Link href={routes.kitchen}>{t('kitchen.detailCtaAll')}</Link>
                </Button>
              </div>
            </div>
          </div>
      ...
      <SectionWrapper
        eyebrow={t('kitchen.detailSectionEyebrow')}
        title={t('kitchen.detailSectionTitle')}
        subtitle={t('kitchen.detailSectionSubtitle')}
      >
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="rounded-lg border border-border/80 bg-card p-6 shadow-sm">
            ...
            <h3 className="mt-4 text-base font-semibold text-primary">
              {t('kitchen.detailAppTitle')}
            </h3>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              {t('kitchen.detailAppBody')}
            </p>
          </div>
          <div className="rounded-lg border border-border/80 bg-card p-6 shadow-sm">
            <FileText className="size-8 text-brand-teal" aria-hidden />
            <h3 className="mt-4 text-base font-semibold text-primary">
              {t('kitchen.detailComplianceTitle')}
            </h3>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              {t('kitchen.detailComplianceBody')}
            </p>
          </div>
          <div className="rounded-lg border border-border/80 bg-card p-6 shadow-sm">
            <Headphones className="size-8 text-brand-teal" aria-hidden />
            <h3 className="mt-4 text-base font-semibold text-primary">
              {t('kitchen.detailSupportTitle')}
            </h3>
